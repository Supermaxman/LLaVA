        ...
        for ex in tqdm(data):
            ...
            try:
                ...
                if type(image_tensor) is list:
                    image_tensor = [image.to(model.device, dtype=torch.bfloat16 if self.load_bf16 else torch.float16) for image in image_tensor]
                else:
                    image_tensor = image_tensor.to(model.device, dtype=torch.bfloat16 if self.load_bf16 else torch.float16)
